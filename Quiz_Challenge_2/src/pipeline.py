...
def run_pipeline(
    ...
    baseline_t = transcriber.transcribe_baseline(audio_path)

    # Temporarily use domain-specific initial_prompt for improved pass
    import whisper as _whisper
    orig_prompt = transcriber._model.transcribe.__defaults__
    improved_result_raw = transcriber._model.transcribe(
        str(audio_path),
    ...
    improved_t = TranscriptionResult(
        text=improved_result_raw["text"].strip(),
        segments=improved_result_raw.get("segments", []),
        language=improved_result_raw.get("language", "en"),
        duration_s=0.0,
