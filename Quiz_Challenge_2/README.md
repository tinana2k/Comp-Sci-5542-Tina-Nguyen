...
### Transcription Quality & Latency (STT Models)

| Model | WER ↓ (Quality) | Inference Time (Latency) | Strengths | Limitations |
|-------|-----------------|--------------------------|-----------|------------|
| Whisper base (baseline) | 8.9% | Faster | Punctuation, multilingual | Acronym errors without domain prompt |
| **Whisper base (improved)** | **3.5%** | Slower (beam search) | Domain prompt + beam_size=5 | GPU recommended for real-time |
| wav2vec2-base-960h | 11.2% | Fastest | Fast CPU inference, Apache-2.0 | ALL-CAPS, no punctuation, English-only |

### Summary Quality (ROUGE vs. Reference)
...
### Sentiment Usefulness & NLP Features

| Feature | Baseline | Improved |
|---------|----------|----------|
| Sentiment Usefulness | Global document sentiment | **Per-sentence sentiment analysis (flags specific negative contexts)** |
